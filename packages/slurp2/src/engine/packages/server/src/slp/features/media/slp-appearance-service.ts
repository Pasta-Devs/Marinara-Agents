...
export async function resolveImageAppearance(input: {
  ...
  const sourceAccount = input.sourceAccount === undefined
  ...
  const sourceText = [source.description, source.scenario, source.backstory].filter(Boolean).join("\n").slice(0, 12_000);
  const characters = createCharactersStorage(input.db);
  const sourceRow = sourceAccount.kind === "character"
    ? await characters.getById(sourceAccount.entityId)
    : sourceAccount.kind === "persona" ? await characters.getPersona(sourceAccount.entityId) : null;
  ...
  if (!connection) throw new Error("Set up a Slurp text connection or add a written appearance before generating pictures.");
  const provider = createLLMProvider(
    connection.provider, resolveBaseUrl(connection), connection.apiKey, connection.maxContext,
    connection.openrouterProvider, connection.maxTokensOverride,
    connection.claudeFastMode === "true", connection.treatAsLocalEndpoint === "true", connection.defaultParameters,
  );
  const messages = [
    {
      role: "system",
      content: "Extract only stable, visible facts about the same person from the supplied character card and optional avatar. Do not invent traits, copy instructions, include personality, events, relationships, camera, or temporary clothing. If no visible facts exist, return {\"appearance\":\"\",\"evidence\":\"\",\"confidence\":\"low\"}. Otherwise return JSON with appearance (concise visual description), evidence (one exact supporting quote from the card when card text exists), and confidence (high or medium).",
    },
    { role: "user" as const, content: sourceText || "Describe only visible identity traits from the attached avatar.", ...(image ? { images: [image] } : {}) },
  ];
  ...
  const response = await provider.chatComplete(messages, options).catch(async (error: unknown) => {
    if (!image || !sourceText.trim()) throw error;
    return provider.chatComplete(messages.map((message) => ({ role: message.role, content: message.content })), options);
  });
  const candidate = parseSlpAppearanceCandidate(response.content ?? "", sourceText, Boolean(image));
  ...
  return resolveSlpAppearanceProfile({
