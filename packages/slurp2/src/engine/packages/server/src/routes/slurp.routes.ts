...
export async function slurpRoutes(app: FastifyInstance) {
  ...
  app.post("/noodler/ads/:id/image", async (req, reply) => {
    ...
    const settings = await noodle.getSettings();
    const outcome = await generateGarnishAdImage(app.db, ads.pool, ad, settings.inlineAdsImageConnectionId ?? settings.imageGenerationConnectionId);
    ...
    try {
      ...
      if (settings.inlineAdsImagesEnabled) {
        for (const ad of items) {
          if (
            (await generateGarnishAdImage(app.db, ads.pool, ad, settings.inlineAdsImageConnectionId ?? settings.imageGenerationConnectionId)) === "generated"
          )
            images += 1;
        }
      }
