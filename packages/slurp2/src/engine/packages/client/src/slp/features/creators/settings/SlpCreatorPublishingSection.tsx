...
import { useCreatorAccounts, useSlpPersonaBackedCreator } from "../slp-creators-hooks";
 ...
 */
export function SlpCreatorPublishingSection({ creator, active }: SlpCreatorSettingsSectionProps) {
  ...
  const accountsQuery = useCreatorAccounts(active);
    ...
    <div className="space-y-5">
      ...
      <SettingsGroup title={t("ui.slurp.settings.creators.postingSchedule")}>
        ...
        {reserveStatusQuery.isLoading ? (
          <div className="flex items-center justify-center gap-2 py-6 text-sm text-[var(--slurp-muted)]" role="status">
          ...
          {postGuidanceQuery.data && (
            ...
            >
              {["", ...SLURP_EXPLICIT_LEVELS].map((level) => {
                ...
                const effective = level || postGuidanceQuery.data.defaults.level || postGuidanceQuery.data.builtInLevel;
                return (
                  <button
                    key={level || "inherit"}
                    type="button"
                    disabled={updatePostGuidance.isPending || postGuidanceQuery.isLoading || postGuidanceQuery.isError}
      ...
      {creator.scheduleStatus && creator.scheduleStatus.state !== "not-applicable" && (
