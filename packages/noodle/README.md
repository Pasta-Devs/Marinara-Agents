...
Noodle timeline refreshes scale their normal post target with the selected non-persona accounts. **Maximum posts per refresh** remains a hard safety ceiling rather than a fixed number of posts to fill, so smaller selections stay bounded and the generated feed can still vary naturally.

NoodleR initially displays 20 feed or Creator-profile posts at a time. **Load more** reveals the next 20 without deleting, truncating, or otherwise limiting stored history. The entry-point badge uses a count-only refresh, so idling elsewhere in Noodle does not reload historical NoodleR posts or media.
