...
- **Hinted**: The Creator is the same person under a stage name, and the connection is a recognizable open secret. Appearance, interests, routines, and selected life details may carry over, and generated text may allude to the other life often enough that a regular follower can put it together. The exact source name and handle are never written, and a guess is never explicitly confirmed. Image generation may use the source character's avatar or preferred full-body image as a reference when avatar references are enabled. Hinted does not promise privacy or anonymity.
...
Profile avatars are presentation for the simulated account. Open Creator profiles reuse the current source avatar. Hinted and Secret profiles start without a source avatar. A change to a more private mode clears a copied source avatar and discards unreleased automatic posts. It is blocked while published identifying text, media, or another avatar still needs review. A shared or recognizable avatar can disclose identity regardless of the selected label. Open and Hinted permit source appearance references for generated post images when enabled. Secret does not. No mode promises anonymity, and the owner must review generated text and images before relying on the intended separation.

Unlock and subscribe show a fictional price. Prices are presentation only: nothing is charged, no balance is shown or tracked, and access is never blocked by a price. A post stores its own unlock price when it is created; posts written before that field existed show the shipped default. Per-Creator subscription prices are not stored yet, so every Creator shows the same default.

Generated output is social-feed copy, not a full roleplay scene. Noodle produces short timeline posts, replies, reposts, likes, and activity summaries. NoodleR produces one concise, editable Creator post with an optional title of at most 200 characters, a default caption body of at most 300 characters, longer bodies only when an explicit format requests them, and an optional short image description when image generation is enabled. Provider output is clipped to these field limits. The owner should use generation guidance for tone, style, maturity, and content boundaries. NoodleR is adults-only and its shipped default guidance is adult-first: flirty, suggestive, and sensual posts are common and explicit posts appear regularly when they suit the Creator, alongside ordinary updates, humor, and project news. Edit the generation guidance to change that balance in either direction.
...
Manual refreshes run generation immediately. Automatic Noodle refreshes update the timeline on the configured local schedule. Automatic NoodleR publishing prepares posts before their release times and publishes them as locked posts while Marinara is running. Enabling synthetic audience activity can add simulated likes, replies, and reposts to released NoodleR posts, locked ones included. A locked post's hidden body and original media never enter a fan prompt: fans react to its title and the fact that it is paid, and their replies stay hidden until the viewer persona unlocks it. Posts held in reserve before their release time stay ineligible. Automation can consume text, vision, and image provider requests, can change roleplay state without another confirmation, and stops when the related setting or package is disabled.
