import { and, desc, eq, gt, inArray, isNull } from "../../db/file-query.js";
import {
  createNoodlePoll,
      ...
      relatedDigests.some(
        (digest) =>
          !parseStringArray(digest.accountIds).every((accountId) =>
            noodleAccountIds.has(accountId),
          ),
      )
    ) {
      return null;
  ...
  return {
    ...
    async patchAccountSettings(
      ...
      return db.transaction(async (tx) => {
        ...
        const row = rows[0];
        if (!row) return null;
        if (row.platform !== "noodle" || input.subtree === "scheduler")
          return null;
        if (input.subtree === "privacy" && input.patch.access !== undefined)
          return null;
        ...
        if (input.subtree === "social") {
          ...
          for (const field of ["noodleFeedSeenAt"] as const) {
    ...
    async listPosts(
      ...
      const noodleAccountIds = (await this.listAccounts()).map(
        (account) => account.id,
      );
        ...
        ? await db
            ...
            .where(
              and(
                ...
                inArray(noodlePosts.authorAccountId, noodleAccountIds),
              ),
            )
    ...
    async deletePost(id: string): Promise<NoodlePost | null> {
      ...
      if (
        interactions.some(
          (interaction) => !noodleAccountIds.has(interaction.actorAccountId),
        )
      )
        return null;
    ...
    async resetTimeline(): Promise<void> {
      ...
      const protectedPostIds = new Set(
        publicInteractions
          .filter(
            (interaction) =>
              !noodleAccountIdSet.has(interaction.actorAccountId),
          )
      ...
      for (const digest of digests) {
        if (
          parseStringArray(digest.accountIds).every((accountId) =>
            noodleAccountIdSet.has(accountId),
          )
        )
          continue;
        if (
    ...
    async listInteractions(
      ...
      return rows
        .filter((row) => noodleAccountIds.has(row.actorAccountId))
        .map(mapInteraction);
    },
    ...
    async deleteInteractionById(id: string): Promise<NoodleInteraction[]> {
      ...
      if (
        deletedRows.some((row) => !noodleAccountIds.has(row.actorAccountId))
      ) {
        return [];
      }
      const relatedDigests = await db
    ...
    async createDigest(input: {
      ...
      if (
        ...
        throw new Error(
          "Public Noodle digests cannot reference non-Noodle accounts.",
        );
      }
      await db.transaction(async (tx) => {
      ...
      return rows
        .filter((row) => {
          ...
          if (row.sourceInteractionId) {
            ...
            if (
              !interaction ||
              !noodleAccountIds.has(interaction.actorAccountId)
            )
              return false;
            const sourcePost = sourcePostById.get(interaction.postId);
            return Boolean(
              sourcePost && noodleAccountIds.has(sourcePost.authorAccountId),
            );
          }
          ...
          if (!sourcePost || !noodleAccountIds.has(sourcePost.authorAccountId))
            return false;
    ...
    async listRefreshRuns(
      options: {
        limit?: number;
        status?: NoodleRefreshRun["status"];
      } = {},
    ) {
    ...
    async finishRefreshRun(
      ...
      try {
        ...
        logger.error(error, "Noodle refresh-run retention cleanup failed");
      }
      return finished;
    },
