...
function readInitialState(): PersistedNoodleState {
  ...
  if (packageState) return packageState as PersistedNoodleState;
  ...
  return {
    noodleNavigation:
      state.noodleNavigation && typeof state.noodleNavigation === "object"
        ? (state.noodleNavigation as NoodleNavigationState)
        : undefined,
    noodleSelectedPersonaId: typeof state.noodleSelectedPersonaId === "string" ? state.noodleSelectedPersonaId : null,
  };
}
...
export const useUIStore = create<NoodlePackageState>((set, get) => ({
  ...
  noodleNavigation: initialState.noodleNavigation ?? { mode: "public", view: "home" },
  ...
  setNoodleNavigation: (noodleNavigation) => {
    ...
    persistNoodleState({ noodleNavigation, noodleSelectedPersonaId: get().noodleSelectedPersonaId });
  },
  setNoodleSelectedPersonaId: (noodleSelectedPersonaId) => {
    set({ noodleSelectedPersonaId });
    persistNoodleState({ noodleNavigation: get().noodleNavigation, noodleSelectedPersonaId });
  },
}));
