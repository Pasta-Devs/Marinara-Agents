...
export async function getLtmExtractionConfig(
  ...
  const templateId =
    resolved.activePromptTemplateIdsByMode[
      mode as keyof typeof resolved.activePromptTemplateIdsByMode
    ];
  if (templateId) {
  ...
  const fallbackPrompt =
    DEFAULT_LTM_EXTRACTION_PROMPTS_BY_MODE[
      mode as keyof typeof DEFAULT_LTM_EXTRACTION_PROMPTS_BY_MODE
    ] ?? DEFAULT_LTM_EXTRACTION_PROMPT;
  return {
    ...resolved,
...
export async function updateLtmExtractionConfig(
  ...
  return withLtmVaultLock(root, async () => {
    ...
    await writeJsonAtomic(
      ltmExtractionConfigPath(root),
      ltmExtractionSettingsSchema.parse(persisted),
    );
