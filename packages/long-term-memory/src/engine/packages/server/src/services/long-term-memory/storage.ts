...
export class LongTermMemoryStorage {
  ...
  async updateNote(id:string,patch:UpdateLtmNotePatch){await this.initializeLtmStore();return withLtmVaultLock(this.root,async()=>{const current=await this.getNote(id);if(!current)throw new Error(`Long-term memory note not found: ${id}`);if(patch.type&&patch.type!==current.type)throw new Error("Changing long-term memory note type is not supported by this package version.");const next=ltmNoteSchema.parse({...current,...patch,id:current.id,type:current.type,createdAt:current.createdAt,updatedAt:nowIso(),version:current.version+1});const event=ltmEventSchema.parse({id:randomUUID(),ts:nowIso(),type:`${next.type}.updated`,target:next.id,payload:{note:next,patch}});await commitLtmMutation(this.root,{files:[{path:notePathForId(next.id,next.type,this.root),before:current,after:next}],events:[event]});return next;});}
  async redirectReferences(fromId:string,toId:string){await this.initializeLtmStore();const from=ltmNoteIdSchema.parse(fromId),to=ltmNoteIdSchema.parse(toId);if(from===to)return{rewrittenNoteCount:0,rewrittenDraftCount:0};return withLtmVaultLock(this.root,async()=>{if(!await this.getNote(to))throw new Error(`Long-term memory replacement note does not exist: ${to}`);const timestamp=nowIso(),notes=await this.listNotes(),noteFiles=[] as Array<{path:string;before:unknown;after:unknown}>;for(const note of notes){const links=note.links.map((link)=>link.target===from?{...link,target:to}:link);if(JSON.stringify(links)===JSON.stringify(note.links))continue;const next=ltmNoteSchema.parse({...note,links,updatedAt:timestamp,version:note.version+1});noteFiles.push({path:notePathForId(next.id,next.type,this.root),before:note,after:next});}const drafts=getLongTermMemoryDirectories(this.root).drafts,draftFiles=[] as Array<{path:string;before:unknown;after:unknown}>;for(const entry of await readdir(drafts,{withFileTypes:true})){if(!entry.isFile()||!entry.name.endsWith(".json"))continue;const path=safeJoin(drafts,entry.name),before=JSON.parse(await readFile(path,"utf8")) as Record<string,unknown>,next={...before};let changed=false;if(next.source&&typeof next.source==="object"&&!Array.isArray(next.source)){const source={...(next.source as Record<string,unknown>)};if(source.sourceNoteId===from){source.sourceNoteId=to;next.source=source;changed=true;}}if(Array.isArray(next.mutations)){const mutations=next.mutations.map((mutation)=>rewriteDraftMutationNoteIds(mutation,from,to));changed ||= JSON.stringify(mutations)!==JSON.stringify(next.mutations);next.mutations=mutations;}if(changed)draftFiles.push({path,before,after:ltmExtractionDraftSchema.parse({...next,updatedAt:timestamp})});}if(noteFiles.length||draftFiles.length)await commitLtmMutation(this.root,{files:[...noteFiles,...draftFiles],events:[]});return{rewrittenNoteCount:noteFiles.length,rewrittenDraftCount:draftFiles.length};});}
  ...
  async deleteNotesPermanently(ids:string[]){await this.initializeLtmStore();const wanted=[...new Set(ids.map((id)=>ltmNoteIdSchema.parse(id)))];return withLtmVaultLock(this.root,async()=>{const notes=await this.listNotes();const lookup=new Map(notes.map((note)=>[note.id,note]));const deletedNotes=wanted.flatMap((id)=>lookup.get(id)?[lookup.get(id)!]:[]);const deletedIds=deletedNotes.map((note)=>note.id);const failedIds=wanted.filter((id)=>!lookup.has(id));if(!deletedNotes.length)return{deletedIds,failedIds,deletedNotes};const deleted=new Set(deletedIds);const repairs=notes.filter((note)=>!deleted.has(note.id)&&note.links.some((link)=>deleted.has(link.target))).map((note)=>ltmNoteSchema.parse({...note,links:note.links.filter((link)=>!deleted.has(link.target)),updatedAt:nowIso(),version:note.version+1}));await commitLtmMutation(this.root,{files:[...deletedNotes.map((note)=>({path:notePathForId(note.id,note.type,this.root),before:note,after:null})),...repairs.map((note)=>({path:notePathForId(note.id,note.type,this.root),before:lookup.get(note.id)!,after:note}))],events:deletedNotes.map((note)=>ltmEventSchema.parse({id:randomUUID(),ts:nowIso(),type:`${note.type}.deleted`,target:note.id,payload:{note}}))});try{const usage=await readLongTermMemoryUsage(this.root);let usageChanged=false;for(const[chatId,chat]of Object.entries(usage.chats)){for(const chunkId of Object.keys(chat.chunks)){if(deleted.has(chat.chunks[chunkId].noteId)){delete chat.chunks[chunkId];usageChanged=true;}}if(Object.keys(chat.chunks).length===0)delete usage.chats[chatId];}if(usageChanged)await writeJsonAtomic(longTermMemoryUsagePath(this.root),usage);}catch(e){if(!isEnoent(e))throw e;}const receiptsDir=getLongTermMemoryDirectories(this.root).receipts;for(const entry of await readdir(receiptsDir,{withFileTypes:true}).catch((e)=>{if(isEnoent(e))return[];throw e;})){if(!entry.isFile()||!entry.name.endsWith(".json"))continue;const receiptPath=safeJoin(receiptsDir,entry.name);try{const receipt=await readJsonFile(receiptPath,null);if(!receipt||!Array.isArray(receipt.chunks))continue;const filtered=receipt.chunks.filter((chunk:any)=>!deleted.has(chunk.noteId));if(filtered.length!==receipt.chunks.length){if(filtered.length===0)await rm(receiptPath,{force:true});else await writeJsonAtomic(receiptPath,{...receipt,chunks:filtered});}}catch{}}return{deletedIds,failedIds,deletedNotes};});}
