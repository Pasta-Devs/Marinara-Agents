  ...
  return async(app)=>{
    ...
    const rebuildAfterMutation = async () => {
      try {
        ...
        logger.warn(error, "[ltm] Deferred index rebuild after maintenance mutation");
        return { status: "deferred" as const, error: error instanceof Error ? error.message : "Index rebuild failed" };
      }
    };
    ...
    app.post<{Body:unknown}>("/notes/permanent-delete",async(request)=>{const body=z.object({ids:z.array(ltmNoteIdSchema).min(1).max(100)}).strict().parse(request.body??{});const result=await storage.deleteNotesPermanently(body.ids);const rebuild=result.deletedIds.length?await rebuildAfterMutation():null;return{deletedIds:result.deletedIds,failedIds:result.failedIds,rebuild};});
    app.delete<{Params:{id:string};Body:unknown}>("/notes/:id/scope",async(request,reply)=>{const id=ltmNoteIdSchema.parse(request.params.id);if(!await storage.getNote(id))return reply.status(404).send({error:"Long-term memory note not found"});const result=await storage.removeNoteFromScope(id,removeScopeBody.parse(request.body??{}));const rebuild=result.changed?await rebuildAfterMutation():null;return result.deleted?{deleted:true,unscoped:false,id,rebuild}:{deleted:false,unscoped:result.changed,id,note:result.note,rebuild};});
