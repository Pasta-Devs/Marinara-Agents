...
type PreparedSource={extractionMethod:"llm";sourceNote:LtmNote;extractionMode:LtmMode;diagnostics:LtmExtractionDiagnostic[];outcome:LtmExtractionOutcome;accounting:LtmExtractionAccounting;response:LtmExtractionResponse;draft:LtmExtractionDraft|null};
...
function canMarkCurrent(prepared:PreparedSource){return prepared.outcome.state==="success"||(prepared.outcome.state==="no_suggestions_created"&&prepared.outcome.droppedUnits===0&&!prepared.diagnostics.some((item)=>item.severity==="error"));}
...
function failed(item:ImportedSourceItem,method:"llm",stage:"extract"|"finalize",error:unknown,isCancelled:boolean,prepared?:PreparedSource):LtmImportedSourceResult{
