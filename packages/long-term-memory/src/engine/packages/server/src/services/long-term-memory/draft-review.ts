...
export async function projectLongTermMemoryDraftReview(options: ProjectLtmDraftReviewOptions = {}): Promise<LtmDraftReviewResponse> {
  ...
  for (const draft of drafts) {
    ...
    for (const diagnostic of draft.diagnostics ?? []) {
      ...
      else if (diagnostic.mutationId && mutationIds.has(diagnostic.mutationId)) rowDiagnostics.set(diagnostic.mutationId, [...(rowDiagnostics.get(diagnostic.mutationId) ?? []), diagnostic]);
      else diagnostics.push(diagnostic);
    }
    if (draft.mutations.length) {
      try {
        const projection = projectLtmDraftOntoNotes({ notes: overlay, mutations: draft.mutations, context: { source: draft.source, scope: draft.scope, modes: draft.modes }, timestamp: nowIso() });
        const mutations = new Map(draft.mutations.map((mutation) => [mutation.id, mutation]));
        for (const note of projection.projections) {
          const rows: LtmDraftReviewMutation[] = note.mutations.map((item) => ({ draftId: draft.id, mutation: mutations.get(item.mutationId)!, disposition: item.disposition, diagnostics: rowDiagnostics.get(item.mutationId) ?? [], changes: item.changes }));
          if (source) {
            const target = source.targets.get(note.noteId);
            if (target) { target.rows.push(...rows); target.title ??= note.after.title; }
            else source.targets.set(note.noteId, { noteId: note.noteId, title: note.after.title, noteType: note.after.type, rows });
          }
        }
        if (!blockReasons.length) for (const note of projection.projections) overlay.set(note.noteId, note.after);
      } catch (error) {
        blockReasons.push({ code: "projection_failed", message: error instanceof Error ? error.message : "Draft projection failed." });
      }
    }
    if (source) source.drafts.push({ draft, freshness, blockReasons, diagnostics, candidateRejections: draft.extractionOutcome?.droppedCandidates ?? [], deduplications });
  }
  const projectedSources: LtmDraftReviewSource[] = [...sources.values()].map((source) => ({ sourceNoteId: source.sourceNoteId, modes: [...source.modes], drafts: source.drafts, targets: [...source.targets.values()] }));
...
function blockReasonsForDraft(draft: LtmDraftReviewDraft["draft"], freshness: LtmDraftFreshness): LtmDraftBlockReason[] {
  const reasons: LtmDraftBlockReason[] = [];
  const reason = freshness === "missing" ? { code: "source_missing" as const, message: "The source note no longer exists." }
    : freshness === "invalid" ? { code: "source_invalid" as const, message: "The source is no longer a source note." }
