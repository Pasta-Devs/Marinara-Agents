...
export async function recoverLtmMutations(root: string) { const dir = getLongTermMemoryDirectories(root).transactions; const entries = await readdir(dir, { withFileTypes: true }).catch((error) => { if (isEnoent(error)) return []; throw error; }); const transactions = await Promise.all(entries.filter((x) => x.isFile() && x.name.endsWith(".json")).map(async (entry) => ltmMutationTransactionSchema.parse(JSON.parse(await readFile(safeJoin(dir, entry.name), "utf8"))))); const committed: LtmMutationTransaction[] = []; for (const tx of transactions.sort((a,b) => a.createdAt.localeCompare(b.createdAt) || a.id.localeCompare(b.id))) { if (tx.status === "committed") { await apply(root, tx, "after"); committed.push(tx); } else { await apply(root, tx, "before"); await remove(root, tx); } await markLtmIndexesDirty(root); } if (committed.length) await rebuildLtmNoteSummary(root); for (const tx of committed) await publish(root, tx, true); }
