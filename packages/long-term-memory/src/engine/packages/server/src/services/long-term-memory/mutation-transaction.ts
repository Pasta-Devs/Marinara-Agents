...
export async function commitLtmMutation(root: string, input: { files: LtmMutationFileChange[]; events?: LtmEvent[] }) {
  ...
  try { await markLtmIndexesDirty(root); const committing = ltmMutationTransactionSchema.parse({ ...prepared, status: "committing" }); await writeJsonAtomic(journalPath(root, prepared.id), committing); await apply(root, committing, "after"); const done = ltmMutationTransactionSchema.parse({ ...committing, status: "committed" }); await writeJsonAtomic(journalPath(root, done.id), done); committed = true; await updateLtmNoteSummary(root, done.id, done.files); await publish(root, done); }
...
export async function recoverLtmMutations(root: string) { const dir = getLongTermMemoryDirectories(root).transactions; const entries = await readdir(dir, { withFileTypes: true }).catch((error) => { if (isEnoent(error)) return []; throw error; }); const transactions: LtmMutationTransaction[] = []; for (const entry of entries.filter((x) => x.isFile() && x.name.endsWith(".json"))) { const path = safeJoin(dir, entry.name); const raw = await readFile(path, "utf8"); try { transactions.push(ltmMutationTransactionSchema.parse(JSON.parse(raw))); } catch (error) { if (!(error instanceof SyntaxError || error instanceof z.ZodError)) throw error; const target = safeJoin(root, `quarantine/transactions/${Date.now()}-${randomUUID()}/${entry.name}`); await mkdir(dirname(target), { recursive: true }); await rename(path, target); logger.warn(error, `[ltm] Quarantined invalid mutation journal ${entry.name}`); } } const committed: LtmMutationTransaction[] = []; for (const tx of transactions.sort((a,b) => a.createdAt.localeCompare(b.createdAt) || a.id.localeCompare(b.id))) { if (tx.status === "committed") { await apply(root, tx, "after"); committed.push(tx); } else { await apply(root, tx, "before"); await remove(root, tx); } await markLtmIndexesDirty(root); } if (committed.length) await rebuildLtmNoteSummary(root); for (const tx of committed) await publish(root, tx, true); }
