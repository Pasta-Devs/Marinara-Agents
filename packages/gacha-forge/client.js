...
<header class="gf-bar">
  ...
  <div class="currencies">
    <div class="currency aet">${wn}<div><div class="value" data-bar-aether>${Fe(o)}</div></div></div>
    <div class="currency">${xn}<div><div class="value" data-bar-funds>${Fe(i)}</div></div></div>
...
`,ka='<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M4 9V4h5M20 9V4h-5M4 15v5h5M20 15v5h-5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',Cn='<svg viewBox="0 0 34 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><rect x="1" y="1" width="12.5" height="22" rx="2"/><rect x="18.5" y="6.5" width="14.5" height="11" rx="1.8"/><path d="M15.4 7.6a6 6 0 0 1 2.6-2.4" stroke-dasharray="2.2 1.8"/><path d="M18.4 4.2l-1 2 2.1.5"/></svg>',Rn=`
  <div class="gf-rot">
    <span class="gf-rot-ph">${Cn}</span>
  ...
  </div>`;function Ln(t){let e=a=>!!(t&&typeof t.has=="function"&&t.has(a));return'<div class="gf-faq">'+xa.map(a=>Bt(a,e(a.id))).join("")+"</div>"}function Hr(t,e){let a=e&&e.onToggle;if(!(!a||!t||typeof t.querySelectorAll!="function"))for(let r of t.querySelectorAll("details[data-help]"))r.addEventListener("toggle",()=>a(r.getAttribute("data-help"),!!r.open))}function Mn(){return'<div class="gf-log">'+Ot().map(t=>'<section class="gf-log-rel'+(t.now?" now":"")+'"><div class="ver">'+u(t.version)+"</div>"+t.body+"</section>").join("")+"</div>"}function Dr(t,e){let a=pt(e&&e.style),r=e&&e.entering?" data-enter":e&&e.swapping?" data-swap":"",s=In[e&&e.onScreen||""]||"";return`
<div class="gf-arena" data-style="${a}">
  ${Rn}
  <aside class="gf-gutter">
    ${s==="left"?"":`<div class="gf-gutter-title">Help</div>${Ln(e&&e.help)}`}
  </aside>
  ...
  <aside class="gf-gutter">
    ${s==="right"?"":`<div class="gf-gutter-title">Changelog</div>${Mn()}`}
  </aside>
</div>
<style>${Ir}</style>`}var On="marinara_admin_secret";function Bn(){try{if(typeof localStorage>"u")return{};let t=(localStorage.getItem(On)||"").trim();return t?{"X-Admin-Secret":t}:{}}catch{return{}}}function ve(t,e){let a=e&&typeof e=="object"?e:{};return fetch(t,{...a,headers:{...Bn(),...a.headers||{}}})}var qr=`
...
`,Hn='<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M4 4h7a2 2 0 0 1 2 2v14a2 2 0 0 0-2-2H4Z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><path d="M20 4h-7a2 2 0 0 0-2 2v14a2 2 0 0 1 2-2h7Z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/></svg>',Dn='<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M4 4l9 9M20 4l-9 9M14.5 14.5 20 20M9.5 14.5 4 20" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',qn='<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M13 2 4 13.5h6L11 22l9-11.5h-6Z" fill="#2E9E7B" stroke="#1C6B54" stroke-width="1.2" stroke-linejoin="round"/></svg>';var _a=t=>String(Math.round(Number(t)||0)).replace(/\B(?=(\d{3})+(?!\d))/gu,",");function $n(t,e){let a=t&&t[e.type==="combat"?"combat":"story"];if(!a)return"";let r=(i,n)=>'<span class="prize">'+ne(i)+n+"</span>",s=[];Number(a.funds)>0&&s.push(r("funds",_a(a.funds))),Number(a.aether)>0&&s.push(r("aether",_a(a.aether)));let o=Number(a.insight&&a.insight.shard)||0;return o>0&&s.push(r("xp",String(o))),Number(a.rank)>0&&s.push(r("rank","+"+_a(a.rank))),`<span class="meta"><span class="cost">${qn}${Number(a.vigor)||0}</span>${s.join("")}</span>`}function jn(t,e,a){return t<a?e.type==="story"?`<button class="act again" type="button" data-replay="${t}"><span>Read again</span><small>&#10004; cleared &middot; free</small></button>`:'<span class="mark done">&#10004; Cleared</span>':t===a?e.type==="story"?'<button class="act play" type="button" data-play><span>Play</span><small>story beat</small></button>':'<button class="act start" type="button" data-start><span>Start</span><small>auto-battle</small></button>':'<span class="mark locked">&#128274; Locked</span>'}function Un(t){return String(Math.round(Number(t)||0)).replace(/\B(?=(\d{3})+(?!\d))/gu,",")}function Wn(t,e){let a=t&&Number.isFinite(Number(t[e]))?Number(t[e]):null;return a===null?"Higher difficulty &middot; harder fight, better rewards":a<=0?"Opening chapter &middot; no CP asked yet":`Recommended CP <b>${Un(a)}</b> &middot; harder fight, better rewards`}function Vr({plan:t,difficulty:e,progress:a,chapterNumber:r=1,pay:s=null,cp:o=null,notice:i=""}){let n=t&&t.title||"Chapter",l=ft(t),d=Ft(e,a,l),h=$r(d),f=De(t,d),m=a[d]||0,v=Qe.map(c=>{let y=c.key===d,E=Ur(c.key,a,l),T=E?"":'<span class="lock">&#128274;</span>',R=c.key==="hard"?"Clear Normal to unlock":"Clear Hard to unlock";return`<button class="diff-pill" type="button" role="tab" aria-selected="${y}" data-diff="${c.key}"${E?"":` disabled title="${R}"`}>${c.label}${T}</button>`}).join(""),b=f.map((c,y)=>{let E=y<m?"done":y===m?"current":"locked",T=c.type==="story"?"Story":"Combat",R=String(y+1).padStart(2,"0");return`<div class="node-row ${E}"><div class="node-rail"><span class="node-idx">${R}</span></div><div class="node-card"><span class="kind">${c.type==="story"?Hn:Dn}${T}</span><span class="title">${u(c.title)}</span>`+$n(s,c)+`</div><div class="node-action">${jn(y,c,m)}</div></div>`}).join(""),w=m>=f.length?`<div class="cap-end">${h.all?"Chapter":u(h.label)} complete</div>`:"";return`
<div class="root">
  <div class="stage"></div>
  <div class="cap">
    <div class="cap-head">
    ...
    <div class="cap-diff">
      ...
      <span class="diff-hint">${Wn(o,d)}</span>
    </div>
...
${Qn(h,!!Xr(s,o))}
        <button class="hm-cta" type="button" data-open-modes>
          ...
          <span class="nodes">${Gn(b,w)}<span>${f?`${w} of ${b} cleared`:"Not started"}</span></span>
...
`;var Il=["Zero","One","Two","Three","Four","Five","Six","Seven","Eight","Nine","Ten"];function Cl(t){let e=Il[t];return e?`Chapter ${e}`:`Chapter ${t}`}var Es=["Reading the scenario\u2026","Forging the chapter\u2026","Writing the story beats\u2026"],Ca=["Reading the scenario\u2026","Summoning the founding cast\u2026","Naming the heroes\u2026"],mt=`
:host { all: initial; display: block; }
*, *::before, *::after { box-sizing: border-box; }
...
</div>`}function Ns({chapterTitle:t,error:e=!1,prologue:a=!1,art:r=null}={}){let s=t&&t.trim()?t.trim():"Chapter One",o=r&&Number(r.total)||0,i=!e&&o>0,n=i?Math.min((Number(r.done)||0)+1,o):0,l=e?"Couldn't write this beat.":i?`${r.name?`Painting ${r.name}`:"Painting this beat's places"}\u2026 ${n}/${o}`:"Generating story\u2026",d=i?"The story is written. It reaches a place this world has never drawn, and that art is being painted now.":"The narrator is writing this beat. It will appear when it's ready.";return`
<div class="root">
  <div class="forge-stage"></div>
  <div class="forge${e?" -error":""}">
    ...
    <div class="forge-center">
      ...
      <h2>${u(s)}</h2>
      <div class="forge-status" aria-live="polite">${u(l)}</div>
...
</div>`;let b=t.kind!=="weapon",w=b&&!!d,c=b&&!t.isProtagonist,y=b,E=b?[["profile","Profile"],["growth","Growth"],["gear","Gear"],...w?[["facets","Facets"]]:[],...y?[["outfits","Outfits"]]:[],...c?[["bond","Bond"]]:[]]:[["profile","Profile"],["growth","Growth"]],T=r;!b&&(T==="bond"||T==="gear")&&(T="profile"),T==="bond"&&!c&&(T="profile"),T==="facets"&&!w&&(T="profile"),T==="outfits"&&!y&&(T="profile");let R=E.map(F=>'<button type="button" role="tab" data-tab="'+F[0]+'" aria-selected="'+(F[0]===T?"true":"false")+'">'+F[1]+"</button>").join(""),W=T==="outfits"?bo(t,h,f,m,v):T==="bond"?Oc(t,a):T==="facets"?Mc(t,d):T==="gear"?Lc(t,i,n,l):T==="growth"?Ao(t,o):Tc(t,e,i&&i.totals),O=b?t.role:t.weaponType+(t.dedicatedTo?" \xB7 for "+he(t.dedicatedTo):""),U='<div class="cp-portrait">'+(Ua(t)?'<img class="cp-photo" src="'+u(Ua(t))+'" alt="" loading="lazy">':b?lc.character:Oe(t.weaponType,"gf-sil"))+'</div><div class="cp-id-top">'+(b&&!t.isProtagonist?'<button class="cp-art-btn" type="button" data-portrait>'+pc+"Portrait</button>":"")+'<button class="cp-fav" type="button" aria-pressed="false" data-fav><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 20S4 14.5 4 9.2A4.2 4.2 0 0 1 12 6a4.2 4.2 0 0 1 8 3.2C20 14.5 12 20 12 20Z" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/></svg></button></div><div class="cp-id-plate"><div class="plate-stars '+ko(t.rarity)+'">'+xo(t.rarity)+"</div><h3>"+u(he(t.name))+'</h3><div class="role">'+u(O)+'</div><div class="chips"><span>Lv '+(Number(e)||1)+"</span>"+(b?'<span class="bond">&#9829; Bond '+(Number(a)||0)+"</span>":"")+'</div><button class="cp-party" type="button"'+(b?" data-set-party":" disabled")+">"+(b?"Set to party":"Equip to a character")+"</button></div>";return`
<div class="root">
  ${Va}
  <div class="stage"></div>
  <section class="screen" data-screen="unit">
...
`;function Pe(t){return String(Math.round(Number(t)||0)).replace(/\B(?=(\d{3})+(?!\d))/gu,",")}function Se(t){return String(t??"").replace(/&/gu,"&amp;").replace(/</gu,"&lt;").replace(/>/gu,"&gt;").replace(/"/gu,"&quot;")}var ii=`
*, *::before, *::after { box-sizing: border-box; }
...
`;function Nt(t){return(Number(t)||0).toLocaleString("en-US")}function Ai(t){if(!t)return[];let e=[];Number(t.funds)>0&&e.push({kind:"funds",qty:Number(t.funds),name:"Funds"}),Number(t.aether)>0&&e.push({kind:"aether",qty:Number(t.aether),name:"Aether"});let a=t.insight||{},r={shard:"Insight Shard",core:"Insight Core",prism:"Insight Prism"};for(let s of["shard","core","prism"])Number(a[s])>0&&e.push({kind:"xp",qty:Number(a[s]),name:r[s]});return e}function Ni(t){return t?t.relic?[]:[{kind:/Funds/i.test(String(t.material))?"funds":/Insight/i.test(String(t.material))?"xp":"asc",qty:Number(t.qty)||0,name:String(t.material||"")}]:[]}var Ii=nt+`
:host { all: initial; display: block; }
*, *::before, *::after { box-sizing: border-box; }
...
`;function hn({runs:t,activeRunId:e}){return`
<div class="rn-root">
  <div class="rn-frame">
    <div class="rn-head">
      <div>
