...
<div class="root">
  ...
  <div class="forge${a?" -error":g?" -paused":""}">
    ...
    <div class="forge-center">
      ...
      <h2>${u(f)}</h2>
      <span class="scenario">${u(d)}</span>
      <div class="forge-status" aria-live="polite">${u(v)}</div>
      <p class="forge-error"${a||g?"":" hidden"}>${g?c.pausedBody:m.head?u(m.head):c.errorBody}</p>
      <button class="forge-retry" type="button"${a||g?"":" hidden"}>${u(g?c.pausedLabel:c.retryLabel||"Retry")}</button>
      ${o?Ds(i):""}
      <p class="forge-why"${a&&m.why?"":" hidden"}>${u(m.why)}</p>
    </div>
