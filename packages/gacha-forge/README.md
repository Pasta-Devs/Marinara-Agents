...
**1.8.6 is one photo from a phone.** Gacha Forge's own "Landscape only" notice was covering Marinara itself: with the game open, the Characters panel and anything else the Engine puts over it were unreachable behind the notice. Measured at 412x915 with a coarse pointer, against the Engine's own panel (fixed, z-50, top = safe-area + 3rem): the notice was anchored to the VIEWPORT, because the arena it hangs from was position: static and container-type: size does not make an element the containing block of an absolute; and its z-index outranked the panel, because a shadow root does not isolate stacking by itself. The arena now declares the package's box twice -- position, so everything absolute is contained, and isolation, so no z-index the package writes can leave it -- and the notice fills that box with inset: 0, which drops the two constants that were copied from the Engine's own layout. The package's rule was "no sheet may declare position: fixed"; it now also proves both declarations, reading the body of each rule.
