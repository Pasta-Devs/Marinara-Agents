...
var c=[{id:"noodle",name:"Noodle",icon:"\u{1F35C}",domain:"noodle.social",description:"The public feed everyone in the story argues on.",preinstalled:!0},{id:"noodler",name:"Noodler",icon:"\u{1F336}\uFE0F",domain:"noodler.social",description:"Noodle's age-gated creator platform. Subscriptions, tips, locked posts.",adult:!0},{id:"search",name:"Search",icon:"\u{1F50D}",domain:"search.web",description:"Look anything up in the story world.",preinstalled:!0},{id:"maps",name:"Maps",icon:"\u{1F5FA}\uFE0F",domain:"maps.web",description:"Where things are, and how far.",preinstalled:!0},{id:"messages",name:"Messages",icon:"\u{1F4AC}",domain:"messages.phone",description:"Texts with characters in the scene.",preinstalled:!0},{id:"gallery",name:"Gallery",icon:"\u{1F5BC}\uFE0F",domain:"gallery.phone",description:"Photos the persona has taken.",preinstalled:!0},{id:"notes",name:"Notes",icon:"\u{1F4DD}",domain:"notes.phone",description:"Scraps, lists, and things worth remembering."},{id:"wallet",name:"Wallet",icon:"\u{1F4B3}",domain:"wallet.phone",description:"Balance, recent charges, and regrettable purchases."},{id:"news",name:"News",icon:"\u{1F4F0}",domain:"news.web",description:"What the world thinks is happening."},{id:"video",name:"Video",icon:"\u25B6\uFE0F",domain:"video.web",description:"Clips, channels, and comment sections."},{id:"music",name:"Music",icon:"\u{1F3B5}",domain:"music.web",description:"Playlists that say too much about the listener."},{id:"forum",name:"Forum",icon:"\u{1F5E3}\uFE0F",domain:"forum.web",description:"Threaded posts from people with strong opinions."},{id:"shop",name:"Shop",icon:"\u{1F4E6}",domain:"shop.web",description:"Listings, reviews, and next-day delivery."},{id:"weather",name:"Weather",icon:"\u{1F326}\uFE0F",domain:"weather.phone",description:"Forecast for wherever the scene is set."}],oe=new Map(c.map(e=>[e.id,e]));function C(e){return oe.get(e)??null}function R(e){let t;try{t=new URL(e,"https://phone.local").hostname.toLowerCase()}catch{return null}return c.find(n=>t===n.domain||t.endsWith(`.${n.domain}`))??null}function $(){return c.filter(e=>e.preinstalled).map(e=>e.id)}function O(){return c}var x=null,I=0;function b(){if(!x)throw new Error("Virtual Phone package runtime is not configured");return x}function z(e){let t=++I;return x=e,()=>{I===t&&(x=null)}}async function E(e,t){let n=b();if(!n.languageModels)throw new Error("This Marinara build does not expose a language-model host to packages.");return n.languageModels.resolveForRequest({connectionId:e,chatConnectionId:t})}var L=["You are writing a page as it appears on a character's phone inside a story. Never break that frame.","Never state or imply that the page, the app, or the wider internet is simulated or AI-generated.","Write as an author inside the story world, not as an assistant.","The screen is 390px wide in portrait. Single column, large tap targets, no side rails, no multi-column dashboards.","Every link must carry a plausible absolute URL on the app's own domain. Never use href='#'."];function se(e){return e.filter(t=>t&&typeof t.content=="string").map(t=>`${t.role||"user"}: ${t.content}`).join(`
`)}function M(e){let t=[];return e.characters?.length&&t.push(`Characters in the scene: ${e.characters.join(", ")}`),e.persona&&t.push(`The phone belongs to: ${e.persona}`),e.chatSummary&&t.push(`Scene: ${e.chatSummary}`),e.worldInfo&&t.push(`World info: ${e.worldInfo}`),e.recentMessages?.length&&t.push(`Recent chat:
${se(e.recentMessages)}`),e.pageHistory&&t.push(`Previous visit to this page: ${e.pageHistory}. Keep names and facts consistent.`),e.navHistory?.length&&t.push(`Navigation path: ${e.navHistory.map(n=>`${n.title||n.url}`).join(" \u2192 ")}`),e.lastAction&&t.push(`Exact user action: ${e.lastAction}`),e.formData&&Object.keys(e.formData).length&&t.push(`Submitted form data: ${JSON.stringify(e.formData)}`),t}var D="observerText must briefly describe what is visible and name the exact link, control, or submitted text the user acted on. Describe the screen, not internal thoughts.";function H(e,t,n){return[{role:"system",content:[`Fill the content slots of the ${e.name} app on a character's phone.`,`App: ${e.name} (${e.domain}) \u2014 URL: ${n.url}`,`Slots to fill: ${t.slots.join(", ")}`,"",t.slotDoc,"",...L,"Do not recreate app chrome, headers, or navigation inside a slot. The template owns all stable structure; supply only the documented content.","Populate every requested slot with complete, realistic content, not placeholders.",D,...M(n),"",'Return only JSON: {"slots":{"slotName":"<HTML string>"},"title":"...","observerText":"...","observerName":"..."}'].join(`
`)},{role:"user",content:`Fill the slots for ${n.url}.`}]}function j(e,t){return[{role:"system",content:[`Generate the ${e.name} app screen at ${t.url} as it appears on a character's phone.`,`App purpose: ${e.description}`,"Return one complete HTML document with semantic markup and all CSS in a <style> element.","Design it like a real, polished native app screen: a sticky top bar, a bottom tab bar with 3-5 tabs, cards or list rows with real spacing, and a dark theme that suits the app.","It must never look like bare unstyled HTML.",...L,D,...M(t),"",'Return only JSON: {"html":"<full document>","title":"...","observerText":"...","observerName":"..."}'].join(`
`)},{role:"user",content:`Render ${t.url} as a polished phone screen.`}]}function _(e){if(typeof e!="string")return null;let t=e.trim().replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/,"");try{let r=JSON.parse(t);if(r&&typeof r=="object")return r}catch{}let n=t.indexOf("{"),o=t.lastIndexOf("}");if(n<0||o<=n)return null;try{let r=JSON.parse(t.slice(n,o+1));return r&&typeof r=="object"?r:null}catch{return null}}var F=`
...
`,U=`
    ...
    </nav>`,V=`Each post is exactly this markup, with your content substituted:
...
`,q=`
    ...
    </nav>`,G=`Each post is exactly this markup, with your content substituted:
...
for text-only posts. The avatar div holds one emoji suiting the creator.`,J=`Noodler is the story world's age-gated creator platform, the adult sibling of Noodle.
...
${V}`,html:`<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Profile / Noodle</title><style>${F}</style></head>
...
${V}`,html:`<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Noodle</title><style>${F}</style></head>
<body><div class="nd-app">
...
${U}
</div></body></html>`},{id:"noodler/profile",appId:"noodler",match:e=>/^\/[^/]+\/?$/.test(e)&&!/^\/(feed|discover|messages|subscriptions)\/?$/.test(e),name:"Noodler creator page",slots:["identity","tiers","posts"],slotDoc:`${J}
...
${G}`,html:`<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
...
<body><div class="nr-app">
  ...
  <main aria-label="Posts"><!-- SLOT: posts --></main>
${q}
</div></body></html>`},{id:"noodler/home",appId:"noodler",match:()=>!0,name:"Noodler feed",slots:["gate","posts"],slotDoc:`${J}
...
${q}
</div></body></html>`}];function K(e,t){let n="/";try{n=new URL(t,"https://phone.local").pathname||"/"}catch{n="/"}return w.find(o=>o.appId===e&&o.match(n))??null}function X(e,t){return e.replace(/<!--\s*SLOT:\s*([\w-]+)\s*-->/g,(n,o)=>{let r=t[o];return typeof r=="string"?r:""})}function Y(e){return[...e.matchAll(/<!--\s*SLOT:\s*([\w-]+)\s*-->/g)].map(t=>t[1])}var re=300*1e3,ae=120,ie=12,h=new Map;function le(e,t){return`${e}\0${t}`}function de(e){let t=h.get(e);return t?Date.now()-t.timestamp>re?(h.delete(e),null):t.value:null}function pe(e,t){for(h.set(e,{timestamp:Date.now(),value:t});h.size>ae;){let n=h.keys().next();if(n.done)break;h.delete(n.value)}}function Q(){h.clear()}function a(e,t=""){return typeof e=="string"?e.trim():t}function d(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:{}}function ce(e,t){let n=e.trim();return/^<!doctype html/i.test(n)||/^<html[\s>]/i.test(n)?n:`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${he(t)}</title></head><body>${n}</body></html>`}function he(e){return e.replace(/[&<>"']/g,t=>t==="&"?"&amp;":t==="<"?"&lt;":t===">"?"&gt;":t==='"'?"&quot;":"&#39;")}async function me(e){let t=b();if(!e||!t.persistence?.getChat)return null;try{return await t.persistence.getChat(e)}catch(n){return t.logger.warn("Virtual Phone could not read the chat: %s",n instanceof Error?n.message:String(n)),null}}async function ue(e,t,n){let o=b(),r=d(e.context),m={url:t,chatSummary:a(r.chatSummary)||void 0,worldInfo:a(r.worldInfo)||void 0,persona:a(r.persona)||void 0,lastAction:a(e.lastAction)||void 0,formData:Object.keys(d(e.formData)).length?d(e.formData):void 0,pageHistory:a(e.pageHistory)||void 0},u=Array.isArray(r.recentMessages)?r.recentMessages:[];u.length&&(m.recentMessages=u.slice(-ie).map(s=>d(s)).filter(s=>typeof s.content=="string").map(s=>({role:a(s.role,"user"),content:String(s.content)})));let l=Array.isArray(e.navHistory)?e.navHistory:[];if(l.length&&(m.navHistory=l.map(s=>d(s)).filter(s=>typeof s.url=="string").slice(-3).map(s=>({url:String(s.url),title:a(s.title)}))),n?.characterIds?.length&&o.resources?.listCharacters)try{let g=(await o.resources.listCharacters(n.characterIds)).map(p=>d(p.data).name).filter(p=>typeof p=="string"&&p.length>0);g.length&&(m.characters=g)}catch(s){o.logger.warn("Virtual Phone could not read chat characters: %s",s instanceof Error?s.message:String(s))}return m}function Z(){return async e=>{e.get("/apps",async()=>({apps:O(),defaults:$()})),e.post("/page",async(t,n)=>{let o=d(t.body),r=b(),m=a(o.appId),u=a(o.url),l=C(m)??(u?R(u):null);if(!l)return n.code(400).send({error:"Unknown phone app."});let s=u||`https://${l.domain}/`,g=a(o.chatId);if(!g)return n.code(400).send({error:"Open a supported chat before using the phone."});let p=le(g,s);if(o.refresh!==!0){let i=de(p);if(i)return{...i,fromCache:!0}}let v=await me(g);if(!v)return n.code(404).send({error:"The active chat could not be found."});let ee=a(o.connectionId)||null,te=a(o.chatConnectionId)||v.connectionId||null,k;try{k=await E(ee,te)}catch(i){return n.code(400).send({error:i instanceof Error?i.message:"Could not resolve a language-model connection."})}let S=await ue(o,s,v),y=K(l.id,s),ne=y?H(l,y,S):j(l,S),T=null;try{let i=k.fitContext(ne);T=(await k.chatComplete(i.messages,{temperature:.9,maxTokens:i.maxTokens,responseFormat:{type:"json_object"}})).content}catch(i){return r.logger.error(i,"Virtual Phone page generation failed"),n.code(502).send({error:"The model could not render that screen."})}let f=_(T);if(!f)return n.code(502).send({error:"The model returned no usable page."});let A=a(f.title)||l.name,P;if(y)P=X(y.html,d(f.slots));else{let i=a(f.html);if(!i)return n.code(502).send({error:"The model returned no page HTML."});P=ce(i,A)}let N={html:P,title:A,observerText:a(f.observerText),observerName:a(f.observerName)||l.name,appId:l.id,url:s};return pe(p,N),N})}}async function ge({api:e,dataDir:t}){let n=z({...e.runtime,dataDir:t});try{let o=await e.registerPrivilegedRoutes(Z(),{prefix:"/api/virtual-phone"});return async()=>{o(),Q(),n()}}catch(o){throw n(),o}}async function fe(){let e=new Set(c.map(t=>t.id));for(let t of w){if(!e.has(t.appId))throw new Error(`Virtual Phone template ${t.id} targets unknown app ${t.appId}`);let n=Y(t.html).sort(),o=[...t.slots].sort();if(n.join(",")!==o.join(","))throw new Error(`Virtual Phone template ${t.id} declares slots [${o}] but its markup has [${n}]`)}return{ok:!0,apps:c.length,templates:w.length}}export{ge as activate,fe as selfCheck};
