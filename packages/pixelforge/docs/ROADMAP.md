...
**Pillar:** substrate for Things and both kinds of Progression. **Unlocks / gates:** S3, S4, P2, P4, P5, W2 — every item that has to remember something between sessions. Nothing in L or E needs it: those stay derived from the seed.
...
- **a sibling world-state block** beside S5, replayed over the compiled world after the rebuild — symmetric, and rewind semantics fall out for free if flags are keyed to the clock they were set at; or
- **host-authoritative**, if the channel that delivers flags can be re-read on load — no save fields at all, at the cost of a dependency; or
- **session-only**, defensible for a first cut, but then S1 itself must say so, because "the GM changed the world" and "until you reload" is a promise with a footnote.

Whichever it is, it is a *different* block from S5 with a *different* lifetime. Conflating them is how a player's inventory gets rewound because a door was reopened.
