...
function catalog() {
  return { schemaVersion: 1, generatedAt: "2026-07-18T00:00:00.000Z", packages: [{ manifest: artifactManifest, category: "misc", artifact: { url: artifactUrl, sha256: sha256(artifactBytes), bytes: artifactBytes.byteLength } }] };
}
...
async function main() {
  globalThis.fetch = (async (input: string | URL | Request) => {
    ...
    if (url === catalogUrl) return new Response(JSON.stringify(catalog()), { status: 200, headers: { "content-type": "application/json" } });
  ...
  try {
    ...
    const note = { id: "world_artifact_lifecycle", title: "Artifact lifecycle fixture", type: "world", status: "active", modes: ["roleplay"], scope: {}, tags: ["artifact_lifecycle"], keywords: ["artifact", "lifecycle"], links: [], sections: { facts: { text: "Exact artifact lifecycle fact survives uninstall and reinstall.", updatedAt: "2026-07-18T00:00:00.000Z" } } };
    const created = await app.inject({ method: "POST", url: "/api/long-term-memory/notes", headers: { "x-marinara-csrf": "1" }, payload: note });
    assert.equal(created.statusCode, 201, created.body);
    const durableRoot = join(dataDir, "long-term-memory");
    const beforeRestart = snapshot(durableRoot);
    await app.close(); app = null;
    app = await buildApp();
    assert.equal((await app.inject({ method: "GET", url: "/api/long-term-memory/notes/world_artifact_lifecycle" })).statusCode, 200);
    assertSnapshot(durableRoot, beforeRestart);
    const backup = await app.inject({ method: "POST", url: "/api/backup/download", headers: { "x-marinara-csrf": "1" } });
    assert.equal(backup.statusCode, 200, backup.body);
    const backupPath = join(dataDir, "ltm-lifecycle-backup.zip");
    mkdirSync(dirname(backupPath), { recursive: true });
    writeFileSync(backupPath, backup.rawPayload);
    assert.match(execFileSync("unzip", ["-Z1", backupPath], { encoding: "utf8" }), /long-term-memory\/vault\/world\/world_artifact_lifecycle\.json/u);
    await app.close(); app = null;
    ...
    assert.equal((await app.inject({ method: "GET", url: "/api/long-term-memory/status" })).statusCode, 200);
    assertSnapshot(durableRoot, beforeRestart);
    console.log("Long-Term Memory exact-artifact lifecycle: install, offline restart, backup inclusion, uninstall, reinstall, and durable-byte preservation ok");
  } finally {
