...
test.describe("package-owned Noodle interface", () => {
  ...
  test("NoodleR bounds historical media requests and loads feed pages deliberately", async ({
    page,
  }, testInfo) => {
    test.skip(
      !testInfo.project.name.includes("desktop"),
      "Request cardinality and both desktop feed rails are covered once on desktop.",
    );
    ...
    try {
      const createPersona = async (name: string) => {
      ...
      const viewerPersona = await createPersona(`Bounded viewer ${stamp}`);
      const externalPersona = await createPersona(`Bounded creator ${stamp}`);
      const hiddenPersona = await createPersona(`Hidden creator ${stamp}`);
      ...
      const accountFor = (personaId: string) => {
        const account = bootstrap.accounts.find(
          (candidate) => candidate.entityId === personaId,
        );
        expect(account).toBeTruthy();
        return account!;
      };
      const viewerAccount = accountFor(viewerPersona.id);
      const externalAccount = accountFor(externalPersona.id);
      const hiddenAccount = accountFor(hiddenPersona.id);
      ...
      const createProfile = async (
        ...
        const response = await page.request.post(
          `/api/noodle/accounts/${sourceAccountId}/noodler`,
          {
            data: {
              stageProfile: {
                displayName: `${label} ${stamp}`,
                handle: `${label.toLowerCase()}_${stamp}`,
                bio: "Temporary bounded-feed browser fixture.",
                stagePersonality: "A fixture used to verify bounded loading.",
                disclosureMode: "secret",
              },
            },
          },
      ...
      const externalProfile = await createProfile(
        externalAccount.id,
        "BoundedCreator",
      );
      await createProfile(viewerAccount.id, "ViewerCreator");
      ...
      const hideResponse = await page.request.patch(
        `/api/noodle/accounts/${hiddenProfile.id}/settings`,
        {
          data: {
            subtree: "privacy",
            patch: {
              access: { hiddenFromAccountIds: [viewerAccount.id] },
            },
          },
        },
      );
      expect(hideResponse.ok()).toBe(true);
      const seenAt = new Date(Date.now() - 1_000).toISOString();
      const seenResponse = await page.request.patch(
        `/api/noodle/accounts/${viewerAccount.id}/settings`,
        {
          data: {
            subtree: "social",
      ...
      expect(seenResponse.ok()).toBe(true);

      const createPost = async (
        targetAccountId: string,
        content: string,
        access: "public" | "locked" = "public",
      ) => {
        const response = await page.request.post("/api/noodle/noodler/posts", {
          data: {
            targetAccountId,
            title: null,
            content,
      ...
      const postTemplate = await createPost(
        externalProfile.id,
        "Bounded public post",
      );
      await createPost(
      ...
      await createPost(profileIds[1]!, "Viewer-owned post");
      await createPost(hiddenProfile.id, "Hidden post");

      const unseenResponse = await page.request.get(
        `/api/noodle/noodler/viewer/unseen-count?personaId=${encodeURIComponent(viewerPersona.id)}`,
      );
      expect(unseenResponse.ok()).toBe(true);
      expect(await unseenResponse.json()).toEqual({ count: 2 });

      const viewerResponse = await page.request.get(
        `/api/noodle/noodler/viewer?personaId=${encodeURIComponent(viewerPersona.id)}`,
      );
      expect(viewerResponse.ok()).toBe(true);
      const realScope = (await viewerResponse.json()) as {
        viewer: Record<string, unknown> & {
          id: string;
          settings: {
            social: { followingAccountIds?: string[] };
          };
        };
        creators: Array<{
      ...
      const realCreator = realScope.creators.find(
        (creator) => creator.profile.id === externalProfile.id,
      );
      expect(realCreator).toBeTruthy();
      const fakeManagedPosts = Array.from({ length: 45 }, (_, index) => ({
        ...postTemplate,
        id: `bounded-media-${stamp}-${index}`,
        authorAccountId: externalProfile.id,
        title: null,
        content: `Bounded image post ${index + 1}`,
        access: "public",
        imageUrl: `/api/noodle/noodler/posts/bounded-media-${stamp}-${index}/media`,
        imagePrompt: null,
        metadata: {},
        createdAt: new Date(Date.now() - index * 1_000).toISOString(),
      }));
      const fakeViewerPosts = fakeManagedPosts.map((post) => ({
        id: post.id,
        authorAccountId: externalProfile.id,
        access: "public",
        locked: false,
        ...
        hasImage: true,
        imageUrl: `${post.imageUrl}?personaId=${encodeURIComponent(viewerPersona.id)}`,
        imagePrompt: null,
        metadata: {},
      ...
      const fakeScope = {
        viewer: {
          ...realScope.viewer,
          settings: {
            ...realScope.viewer.settings,
            social: {
              ...realScope.viewer.settings.social,
              followingAccountIds: [externalProfile.id],
            },
          },
        },
        creators: [
          {
            ...realCreator!,
            followed: true,
            posts: fakeViewerPosts,
          },
        ],
      };

      await page.route(
        "**/api/noodle/noodler/viewer?personaId=*",
        (route) =>
          route.fulfill({
            status: 200,
      ...
      await page.route(
        `**/api/noodle/noodler/accounts/${externalProfile.id}/posts`,
        (route) =>
          route.fulfill({
            status: 200,
            contentType: "application/json",
            body: JSON.stringify(fakeManagedPosts),
          }),
      );

      const requests = { unseen: 0, viewer: 0, media: 0 };
      const rateLimited: string[] = [];
      page.on("request", (request) => {
        const url = new URL(request.url());
        if (url.pathname === "/api/noodle/noodler/viewer/unseen-count")
          requests.unseen += 1;
        if (url.pathname === "/api/noodle/noodler/viewer")
          requests.viewer += 1;
        if (/^\/api\/noodle\/noodler\/posts\/bounded-media-[^/]+\/media$/u.test(url.pathname))
          requests.media += 1;
      });
      ...
      await expect.poll(() => requests.unseen).toBeGreaterThanOrEqual(1);
      await expect
        .poll(() => requests.unseen, { timeout: 35_000 })
        .toBeGreaterThanOrEqual(2);
      expect(requests.viewer).toBe(0);
      expect(requests.media).toBe(0);

      const noodle = page.locator('[data-component="NoodleView"]');
      await noodle.getByRole("tab", { name: "NoodleR", exact: true }).click();
      const renderedPosts = noodle.locator(
        '[data-noodle-post-id^="bounded-media-"]',
      );
      await expect(renderedPosts).toHaveCount(20);
      await expect.poll(() => requests.media).toBe(20);

      const mediaBeforeRefresh = requests.media;
      const viewerRefresh = page.waitForResponse(
        (response) =>
          new URL(response.url()).pathname ===
          "/api/noodle/noodler/viewer",
      );
      await noodle
      ...
      expect(requests.media).toBe(mediaBeforeRefresh);

      const feedLoadMore = noodle.locator(
        '[data-component="NoodlerHome.LoadMoreFeed"]',
      );
      await expect(feedLoadMore).toHaveText("Load more (20 of 45)");
      await feedLoadMore.click();
      await expect(renderedPosts).toHaveCount(40);
      await expect.poll(() => requests.media).toBe(40);

      await noodle.getByRole("tab", { name: "All creators" }).click();
      await expect(renderedPosts).toHaveCount(20);
      await expect(feedLoadMore).toHaveText("Load more (20 of 45)");

      const search = noodle.getByPlaceholder("Search posts or @creators");
      await search.fill("Bounded image post");
      await expect(renderedPosts).toHaveCount(20);
      await expect(feedLoadMore).toHaveText("Load more (20 of 45)");
      await search.fill("");
      await expect(renderedPosts).toHaveCount(20);
      ...
      await expect(renderedPosts).toHaveCount(40);
      await feedLoadMore.click();
      await expect(renderedPosts).toHaveCount(45);

      await renderedPosts
        .first()
        .getByRole("button", {
          name: externalProfile.displayName,
          exact: true,
        })
        .click();
      await expect(renderedPosts).toHaveCount(20);
      const profileLoadMore = noodle.locator(
        '[data-component="NoodlerProfile.LoadMorePosts"]',
      );
      await expect(profileLoadMore).toHaveText("Load more (20 of 45)");
      await profileLoadMore.click();
      await expect(renderedPosts).toHaveCount(40);
      await profileLoadMore.click();
      await expect(renderedPosts).toHaveCount(45);

      await noodle.getByRole("tab", { name: "Media", exact: true }).click();
      await expect(renderedPosts).toHaveCount(20);
      await expect(profileLoadMore).toHaveText("Load more (20 of 45)");
      expect(requests.media).toBeLessThan(200);
      ...
      for (const profileId of profileIds.reverse()) {
        await page.request
          .delete(`/api/noodle/noodler/accounts/${profileId}`, {
            timeout: 5_000,
          })
          .catch(() => undefined);
      }
      ...
      for (const personaId of personaIds.reverse()) {
        await page.request
