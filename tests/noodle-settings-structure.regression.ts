...
async function main() {
  const [noodleHome, slurpHome, slurpSettings, slurpTypes, slurpStore] = await Promise.all([
    ...
    readFile("packages/slurp/src/engine/packages/client/src/components/slurp/NoodlerPublishingSettings.tsx", "utf8"),
  ...
  assert.doesNotMatch(noodleHome, /enableNoodler|NoodlerPublishingSettings|NoodlerAgeGate/u);
  assert.match(slurpSettings, /useUpdateNoodlerAutoPosting|useUpdateNoodlerFanActivity/u);
  assert.match(slurpHome, /NoodlerBulkCreatePanel/u);
  assert.match(slurpTypes, /mode: "creator-settings"/u);
  assert.match(slurpTypes, /sourceAccountId: string/u);
  assert.match(slurpStore, /marinara:slurp:ui/u);
