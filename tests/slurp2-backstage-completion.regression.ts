...
assert.match(
  ...
  "selecting a Creator opens its settings, which start on the safe Profile tab",
);
assert.match(
  creators,
  /tab: "profile",\s*\n?\s*settingKey/u,
  "the settings store opens on Profile (Identity first) unless the caller asks for another tab",
);
