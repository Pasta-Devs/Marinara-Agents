...
assert.match(
  service,
  /\.orderBy\(asc\(slurpPendingText\.priority\), desc\(slurpPendingText\.createdAt\)\)\s*\.limit\(limit\)/u,
);
...
assert.match(service, /try \{[\s\S]*rows = await db[\s\S]*\.from\(slurpPendingText\)/u);
