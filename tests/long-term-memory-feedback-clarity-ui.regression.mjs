...
assert.ok(vault.lastIndexOf("data-ltm-availability-pills") < vault.lastIndexOf("data-ltm-availability-picker"));
...
assert.match(vault, /scopeMode/u);
assert.match(vault, /scopeChats/u);
assert.match(vault, /changeScopeMode/u);
assert.match(vault, /chatModes/u);
assert.match(vault, /allChatModes/u);
...
assert.match(targetPicker, /data-ltm-availability-tabs[\s\S]*border border-\[var\(--marinara-editor-divider\)\]/u);
assert.match(targetPicker, /data-ltm-availability-tab=\{kind\}[\s\S]*border border-\[var\(--marinara-editor-divider\)\]/u);
...
assert.match(vault, /filterByType[\s\S]*className=\{`\$\{inputClass\} text-xs`\}/u);
...
assert.equal(locale["ui.longTermMemory.memoryvault.allChatModes"], "All chat modes");
