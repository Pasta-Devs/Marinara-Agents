...
assert.match(chatSettings, /min-h-8 w-full[\s\S]*text-\[0\.625rem\]/u);
assert.doesNotMatch(chatSettings, /space-y-2 px-2/u);

assert.match(controls, /compactInputClass[\s\S]*min-h-8/u);
...
process.stdout.write(
  "Long-Term Memory chat settings UI regression: compact spacing, controls, and typography ok\n",
);
