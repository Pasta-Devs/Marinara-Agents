...
| Relationship Tracker | [`relationship-tracker`](packages/relationship-tracker/manifest.json) | Staging only | Tracks character-card relationships and each character's one-way perception of the active persona in Roleplay group chats, drawn as an editable relationship web in the Tracker Panel. Conservative automatic updates, bounded Update from History, manual locks, and All or Scene-only prompt injection. Needs an Engine with Capability API 1.14. |
