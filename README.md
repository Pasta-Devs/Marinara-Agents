...
| Noodle | [`noodle`](packages/noodle/manifest.json) | Explore the public Noodle timeline as an optional local social world, available after installation from **Home → Noodle**. |
| Slurp | [`slurp`](packages/slurp/manifest.json) | Create local Creator profiles from Engine characters or personas, publish public or locked NoodleR posts, and simulate audience activity from **Home → Slurp**. |
