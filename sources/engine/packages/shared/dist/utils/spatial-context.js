...
function buildSpatialLocationIndex(definition) {
...
function getSpatialDescendantIds(definition, locationId) {
...
function wouldCreateSpatialCycle(definition, locationId, parentId) {
  if (parentId === locationId) return true;
  return getSpatialDescendantIds(definition, locationId).has(parentId);
}
function resolveSpatialLocationDepth(definition, location) {
...
function spatialRadialPlacement(index, count, radius = count <= 6 ? 34 : 40) {
...
function compareSpatialLocations(left, right) {
...
function compareDestinations(left, right) {
  return left.sortOrder - right.sortOrder || compareText(left.name, right.name) || compareText(left.id, right.id) || compareText(left.relation, right.relation);
}
...
function resolveSpatialBreadcrumb(definition, locationId) {
...
function destinationFromLocation(location, relation, label) {
  return {
    id: location.id,
    name: location.name,
    kind: location.kind,
...
function resolveSpatialDestinations(definition, currentLocationId) {
  ...
  const add = (location, relation, label) => {
...
function resolveSpatialRoute(definition, currentLocationId, targetLocationId) {
  ...
  const visited = /* @__PURE__ */ new Set([currentLocationId]);
...
function validateSpatialArchive(definition, locationId, options) {
  const byId = buildSpatialLocationIndex(definition);
  if (!byId.has(locationId)) {
    return { ok: false, code: "spatial_location_missing", message: "The location does not exist." };
  }
  if (definition.startingLocationId === locationId && !validReplacement(byId, locationId, options.replacementLocationId)) {
    return {
      ok: false,
...
function validateSpatialTransition(definition, currentLocationId, request) {
  ...
  if (request.travelMode) {
    ...
    const destination2 = request.travelMode === "step_by_step" ? destinations.find((candidate) => candidate.id === acceptedDestinationId) : destinationFromLocation(byId.get(acceptedDestinationId), "link");
    if (!destination2) {
      return {
        ok: false,
        code: "spatial_destination_unreachable",
    ...
    return { ok: true, destination: destination2, travel };
  }
